...
async function validateExecutable(binDir, executable, os) {
    try {
...
async function validateVimDir(path) {
    let entries;
    try {
    ...
    throw new Error(`Validation failed! $VIM directory ${path} contains no $VIMRUNTIME directory: ${JSON.stringify(entries)}`);
}
export async function validateInstallation(installed, os) {
